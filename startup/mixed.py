...
import time
def start_process_mixed(road_type:str,osm_fclass,qq_subtype,CITY:str,smooth_level:int = 30,extend_distance:int = 100,spike_keep:int = 500):
    ...
    return f"{OUTPUT_PATH}/f'road_result_{CITY}_{road_type}.shp'"

def mixed_start(i,smooth_level:int = 30,extend_distance:int = 100,spike_keep:int = 500):
    res = start_process_mixed('mixed',OSM_MAIN_FCLASS,QQ_MAIN_SUBTYPE,i,smooth_level,extend_distance,spike_keep)
    deleteGDBFile()
    return res
